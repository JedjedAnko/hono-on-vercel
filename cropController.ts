...
export const cropController = {
    ...
    create: async (c: Context) => {
        try {
            const db_models = c.get('db_models') as Models["models"];
            const body = await c.req.json() as cropform;
            console.log(body)
            let create = await db_models.crop_model.create(body);
    ...
    patch: async (c: Context) => {
        try {
            ...
            const body = await c.req.json() as cropform;
            const {_id, ...data} = body;
            let patch = await db_models.crop_model.findOneAndUpdate({_id: new mongoose.Types.ObjectId(_id)}, data, {new: true});
    ...
    delete: async (c: Context) => {
        try {
            ...
            const db_models = c.get('db_models') as Models["models"];
            let remove = await db_models.crop_model.findByIdAndDelete({_id: new mongoose.Types.ObjectId(id)});
