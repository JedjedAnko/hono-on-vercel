...
export const machineryController = {
    ...
    create: async (c: Context) => {
        try {
            const db_models = c.get('db_models') as Models["models"];
            const body = await c.req.json() as machineryform;
            console.log(body)
            let create = await db_models.machinery_model.create(body);
    ...
    patch: async (c: Context) => {
        try {
            ...
            const body = await c.req.json() as machineryform;
            const {_id, ...data} = body;
            let patch = await db_models.machinery_model.findOneAndUpdate({_id: new mongoose.Types.ObjectId(_id)}, data, {new: true});
    ...
    delete: async (c: Context) => {
        try {
            ...
            const db_models = c.get('db_models') as Models["models"];
            let remove = await db_models.machinery_model.findByIdAndDelete({_id: new mongoose.Types.ObjectId(id)});
